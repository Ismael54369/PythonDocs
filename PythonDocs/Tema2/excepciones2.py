import uuid # Para generar IDs únicos si el usuario no introduce uno
...
class Material:
    ...
    def mostrar(self):
        ...
        print(f"ID: {self.id} | Título: {self.titulo} | Autor: {self.autor} | Año: {self.anio_publicacion}", end="")
...
class Libro(Material):
    ...
    def mostrar(self):
        ...
        print("--- LIBRO ---")
        super().mostrar()
        print(f" | Género: {self.genero} | Páginas: {self.num_paginas}")
...
class Revista(Material):
    ...
    def mostrar(self):
        ...
        print("--- REVISTA ---")
        super().mostrar()
        print(f" | Edición: {self.num_edicion} | Mes: {self.mes_publicacion}")

# --- GESTIÓN DE LA BIBLIOTECA ---

materiales = {} # Diccionario para almacenar los materiales por ID
ids_existentes = set() # Usamos un conjunto para una búsqueda más eficiente de IDs
...
def agregar_material():
    ...
    while True:
        id_material = input("Introduce el ID del material (dejar en blanco para ID automático): ")
        if not id_material:
            id_material = str(uuid.uuid4())[:8] # Genera un ID corto y único
...
def buscar_material_por_id():
    ...
    id_buscado = input("Introduce el ID del material a buscar: ")
    
    material = materiales.get(id_buscado) # .get() es más seguro que acceso directo
    if material:
        material.mostrar()
    else:
...
def eliminar_material():
    ...
    print("\n--- Eliminar Material ---")
    id_a_eliminar = input("Introduce el ID del material a eliminar: ")
...
def generar_estadisticas():
    ...
    promedio_paginas = total_paginas_libros / num_libros if num_libros > 0 else 0
...
if __name__ == "__main__":
    ...
    # Para que el programa no empiece vacío, podemos añadir algunos materiales de prueba.
    try:
        libro_ejemplo = Libro("L001", "Cien Años de Soledad", "Gabriel García Márquez", 1967, "FICCION", 496)
        revista_ejemplo = Revista("R001", "National Geographic", "Varios", 2023, 12, "MAYO")
        materiales = {
            "L001": libro_ejemplo,
            "R001": revista_ejemplo
        }
        ids_existentes = {"L001", "R001"}
        print("INFO: Se han cargado datos de ejemplo.")
